...
BEGIN
  SELECT CASE WHEN NOT EXISTS (
    SELECT 1 FROM academy_public_recommendation_sets s
    WHERE s.id=NEW.set_id AND s.tenant_id=NEW.tenant_id
  ) THEN RAISE(ABORT,'recommendation item tenant mismatch') END;

  SELECT CASE WHEN NEW.item_type='course' AND NOT EXISTS (
    SELECT 1 FROM academy_courses x WHERE x.id=NEW.item_ref AND x.tenant_id=NEW.tenant_id
  ) THEN RAISE(ABORT,'recommendation course tenant mismatch') END;
  SELECT CASE WHEN NEW.item_type='path' AND NOT EXISTS (
    SELECT 1 FROM academy_public_learning_paths x WHERE x.id=NEW.item_ref AND x.tenant_id=NEW.tenant_id
  ) THEN RAISE(ABORT,'recommendation path tenant mismatch') END;
  SELECT CASE WHEN NEW.item_type='instructor' AND NOT EXISTS (
    SELECT 1 FROM academy_instructor_public_profiles x WHERE x.instructor_id=NEW.item_ref AND x.tenant_id=NEW.tenant_id
  ) THEN RAISE(ABORT,'recommendation instructor tenant mismatch') END;
  SELECT CASE WHEN NEW.item_type='event' AND NOT EXISTS (
    SELECT 1 FROM academy_events x WHERE x.id=NEW.item_ref AND x.tenant_id=NEW.tenant_id
  ) THEN RAISE(ABORT,'recommendation event tenant mismatch') END;
  SELECT CASE WHEN NEW.item_type='plan' AND NOT EXISTS (
    SELECT 1 FROM academy_plans x WHERE x.id=NEW.item_ref AND x.tenant_id=NEW.tenant_id
  ) THEN RAISE(ABORT,'recommendation plan tenant mismatch') END;
  SELECT CASE WHEN NEW.item_type='partner' AND NOT EXISTS (
    SELECT 1 FROM academy_public_partners x WHERE x.id=NEW.item_ref AND x.tenant_id=NEW.tenant_id
  ) THEN RAISE(ABORT,'recommendation partner tenant mismatch') END;
  SELECT CASE WHEN NEW.item_type='bundle' AND NOT EXISTS (
    SELECT 1 FROM academy_public_bundles x WHERE x.id=NEW.item_ref AND x.tenant_id=NEW.tenant_id
...
BEGIN
  SELECT CASE WHEN NOT EXISTS (
    SELECT 1 FROM academy_public_recommendation_items i WHERE i.tenant_id=NEW.tenant_id AND i.set_id=NEW.id
  ...
  SELECT CASE WHEN EXISTS (
    SELECT 1 FROM academy_public_recommendation_items i
    LEFT JOIN academy_public_learning_paths p ON i.item_type='path' AND p.id=i.item_ref AND p.tenant_id=i.tenant_id AND p.visibility='public'
  ...
  SELECT CASE WHEN EXISTS (
    SELECT 1 FROM academy_public_recommendation_items i
    LEFT JOIN academy_events e ON i.item_type='event' AND e.id=i.item_ref AND e.tenant_id=i.tenant_id AND e.status='published'
  ...
  SELECT CASE WHEN EXISTS (
    SELECT 1 FROM academy_public_recommendation_items i
    LEFT JOIN academy_plans p ON i.item_type='plan' AND p.id=i.item_ref AND p.tenant_id=i.tenant_id AND p.status='public'
  ...
  SELECT CASE WHEN EXISTS (
    SELECT 1 FROM academy_public_recommendation_items i
    LEFT JOIN academy_public_partners p ON i.item_type='partner' AND p.id=i.item_ref AND p.tenant_id=i.tenant_id AND p.status='public'
  ...
  SELECT CASE WHEN EXISTS (
    SELECT 1 FROM academy_public_recommendation_items i
    LEFT JOIN academy_public_bundles b ON i.item_type='bundle' AND b.id=i.item_ref AND b.tenant_id=i.tenant_id AND b.status='public'
