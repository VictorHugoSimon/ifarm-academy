...
try:
    conn.execute('''INSERT INTO academy_payment_events
      (id,tenant_id,checkout_session_id,provider,provider_event_id,event_type,provider_payment_id,provider_subscription_id,amount_cents,currency,payload_hash,verified_at,received_at,period_start,period_end)
      VALUES ('EVTX','T2','CHK1','mercado_pago','evt-x','confirmed','pay-x','sub-x',5990,'BRL',?,?,?,?,?)''',('a'*64,now,now,now,period_end))
...
conn.execute('''INSERT INTO academy_payment_events
  (id,tenant_id,checkout_session_id,provider,provider_event_id,event_type,provider_payment_id,provider_subscription_id,amount_cents,currency,payload_hash,verified_at,received_at,period_start,period_end)
  VALUES ('EVT1','T1','CHK1','mercado_pago','evt-1','confirmed','pay-1','sub-1',5990,'BRL',?,?,?,?,?)''',('b'*64,now,now,now,period_end))
...
try:
    conn.execute('''INSERT INTO academy_payment_events
      (id,tenant_id,checkout_session_id,provider,provider_event_id,event_type,provider_payment_id,provider_subscription_id,amount_cents,currency,payload_hash,verified_at,received_at,period_start,period_end)
      VALUES ('EVT_DUP','T1','CHK1','mercado_pago','evt-1','confirmed','pay-1','sub-1',5990,'BRL',?,?,?,?,?)''',('c'*64,now,now,now,period_end))
