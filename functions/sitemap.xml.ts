...
export const onRequestGet=async({env,request}:{env:Env;request:Request})=>{
  ...
  const [courses,paths,instructors,events,plans,partners,bundles]=await Promise.all([
    db.prepare(`SELECT cp.slug,cp.updated_at FROM academy_course_public_profiles cp
    ...
    db.prepare(`SELECT p.slug,p.updated_at FROM academy_public_learning_paths p
    ...
    db.prepare(`SELECT p.slug,p.updated_at FROM academy_instructor_public_profiles p
    ...
    db.prepare(`SELECT id,updated_at FROM academy_events WHERE tenant_id=? AND status='published' AND datetime(ends_at)>=datetime('now') ORDER BY starts_at`).bind(context.tenantId).all(),
    db.prepare(`SELECT slug,updated_at FROM academy_plans WHERE tenant_id=? AND status='public' ORDER BY slug`).bind(context.tenantId).all(),
    db.prepare(`SELECT slug,updated_at FROM academy_public_partners WHERE tenant_id=? AND status='public' ORDER BY slug`).bind(context.tenantId).all(),
    db.prepare(`SELECT slug,updated_at FROM academy_public_bundles WHERE tenant_id=? AND status='public' ORDER BY slug`).bind(context.tenantId).all(),
  ])
  ...
  for(const row of rows(courses))entries.push({path:`/courses/${row.slug}`,lastModified:iso(row.updated_at),changeFrequency:'weekly',priority:.8})
  for(const row of rows(paths))entries.push({path:`/paths/${row.slug}`,lastModified:iso(row.updated_at),changeFrequency:'weekly',priority:.7})
  for(const row of rows(instructors))entries.push({path:`/instructors/${row.slug}`,lastModified:iso(row.updated_at),changeFrequency:'monthly',priority:.6})
  for(const row of rows(events))entries.push({path:`/events/${encodeURIComponent(String(row.id))}`,lastModified:iso(row.updated_at),changeFrequency:'daily',priority:.7})
  for(const row of rows(plans))entries.push({path:`/plans/${row.slug}`,lastModified:iso(row.updated_at),changeFrequency:'weekly',priority:.7})
  for(const row of rows(partners))entries.push({path:`/partners/${row.slug}`,lastModified:iso(row.updated_at),changeFrequency:'monthly',priority:.6})
  for(const row of rows(bundles))entries.push({path:`/bundles/${row.slug}`,lastModified:iso(row.updated_at),changeFrequency:'weekly',priority:.7})
