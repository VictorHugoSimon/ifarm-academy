...
export const onRequestPost = async ({ env, request }: { env: Env; request: Request }) => {
  ...
  if (eventType === 'confirmed') {
    const canonicalTimingEvidence = canonical.periodStart ?? canonical.occurredAt
    if (!canonical.providerPaymentId || !providerSubscriptionId || !canonicalTimingEvidence) {
      await db.prepare(`UPDATE academy_payment_webhook_receipts SET detail_code='confirmed_waiting_canonical_timing_evidence' WHERE id=?`)
        .bind(receipt.id).run()
      return json({ accepted: true, status: 'canonical_verified', receiptId: receipt.id, processing: 'waiting_canonical_timing_evidence' })
    }
  }
  ...
  try {
    const result = await processVerifiedPaymentEvent(db, {
      ...
      event: {
        ...
        providerOccurredAt: canonical.occurredAt,
