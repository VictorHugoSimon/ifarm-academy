...
function reasonText(value:unknown){
  ...
  return reason.slice(0,500)||null
}

function mapOpportunity(row:any,state:any){return{
  ...
  consentVersion:row.consent_version??null,consentRecordedAt:String(row.consent_recorded_at),stage:String(row.stage),
...
async function loadPrivacyView(db:any,tenantId:string,userId:string){
  ...
  const result=await db.prepare(`SELECT * FROM academy_commercial_opportunities
    WHERE tenant_id=? AND user_id=? ORDER BY created_at DESC LIMIT 100`).bind(tenantId,userId).all()
...
export const onRequestPost=async({env,request}:{env:Env;request:Request})=>{
  ...
  ])}catch{return json({error:'A reautorização exige uma regra explícita ainda ativa e a versão atual do consentimento'},409)}
  return json({data:await loadPrivacyView(db,auth.tenantId,auth.userId)})
}
