...
export const onRequestGet = async ({ env, request }: { env: Env; request: Request }) => {
  ...
  const links = await db.prepare(`
    SELECT
      link.path_assignment_id,
      pc.course_id,
      pc.required,
      pc.renewal_months,
      c.title course_title,
      ca.id course_assignment_id,
      ca.status course_assignment_status,
      ca.learning_cycle_id,
      lc.cycle_number,
      lc.status cycle_status,
      cert.public_code certificate_code,
      COALESCE((
        SELECT ROUND(AVG(COALESCE(pr.progress_percent,0)))
        FROM academy_course_lessons l
        LEFT JOIN academy_progress pr
          ON pr.tenant_id=l.tenant_id AND pr.course_id=l.course_id
          AND pr.lesson_id=l.id AND pr.student_id=m.user_id
          AND pr.cycle_id=ca.learning_cycle_id
        WHERE l.tenant_id=ca.tenant_id AND l.course_id=ca.course_id AND l.required=1
      ),0) progress_percent
    ...
    LEFT JOIN academy_learning_cycles lc ON lc.tenant_id=ca.tenant_id AND lc.id=ca.learning_cycle_id
    LEFT JOIN academy_certificates cert ON cert.tenant_id=ca.tenant_id
      AND cert.student_id=m.user_id AND cert.course_id=ca.course_id
      AND cert.cycle_id=ca.learning_cycle_id AND cert.status='valid'
  ...
  for (const row of links.results as any[]) {
    ...
    const completed = String(row.course_assignment_status) === 'completed' || String(row.cycle_status ?? '') === 'completed'
    list.push({
      courseId: row.course_id,
      courseTitle: row.course_title,
      courseAssignmentId: row.course_assignment_id,
      ...
      progressPercent: completed ? 100 : Math.max(0, Math.min(100, Number(row.progress_percent ?? 0))),
      completed,
  ...
  return json({ data: (assignments.results as any[]).map((row) => {
    ...
    return {
      ...
      status: row.status,
      effectiveStatus: cancelled ? 'cancelled' : complete ? 'completed' : progress > 0 ? 'in_progress' : 'assigned',
...
export const onRequestPost = async ({ env, request }: { env: Env; request: Request }) => {
  ...
  const pathCourses = await db.prepare(`
    SELECT pc.*, c.status course_status
    FROM academy_company_learning_path_courses pc
    JOIN academy_courses c ON c.tenant_id=pc.tenant_id AND c.id=pc.course_id
    WHERE pc.tenant_id=? AND pc.company_id=? AND pc.path_id=? ORDER BY pc.position
  ...
  const pathAssignmentId = crypto.randomUUID()
  const now = new Date().toISOString()
  const statements: any[] = []
  let allRequiredCompleted = true
  statements.push(db.prepare(`
    INSERT INTO academy_company_path_assignments (
      id,tenant_id,company_id,path_id,member_id,status,due_at,assigned_by,assigned_at,completed_at,updated_at
  ...
  for (const row of pathCourses.results as any[]) {
    const assignment = await ensureEnterpriseCourseAssignment(db, {
      ...
      source: `company_path:${pathAssignmentId}`,
      actorId: auth.userId,
      now,
    })
  ...
  if (allRequiredCompleted) {
    statements.push(db.prepare(`UPDATE academy_company_path_assignments SET status='completed',completed_at=?,updated_at=? WHERE tenant_id=? AND id=?`)
      .bind(now, now, auth.tenantId, pathAssignmentId))
  }
  statements.push(auditStatement(db, auth, {
    action: 'company_learning_path.assigned',
    resourceType: 'company_path_assignment',
    resourceId: pathAssignmentId,
  ...
  return json({ data: {
    ...
    status: allRequiredCompleted ? 'completed' : 'assigned',
    dueAt: normalizedDueAt,
    assignedAt: now,
  } }, 201)
}
