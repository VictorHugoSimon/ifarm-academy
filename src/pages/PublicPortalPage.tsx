...
export function PublicPortalPage() {
  ...
  return <div className="publicPortal" style={brandStyle(brand)}>
    ...
    {!loading && !error && view === 'home' && <main>
      <section className="publicHero"><div><small>Educação conectada ao agro real</small><h1>Conhecimento para transformar decisões no campo.</h1><p>Cursos, trilhas, especialistas, treinamentos corporativos, experiências práticas e tecnologia integrados ao ecossistema iFarm.</p><div className="publicHeroActions"><button className="primary" onClick={() => go('/courses')}>Explorar cursos</button><button onClick={() => go('/paths')}>Ver trilhas</button><button onClick={() => go('/events')}>Ver eventos</button></div></div><div className="publicHeroPanel"><span>Smart Farm Experience</span><strong>Aprender. Praticar. Certificar.</strong><p>Conteúdo digital conectado a dias de campo, demonstrações e capacitação prática.</p></div></section>
