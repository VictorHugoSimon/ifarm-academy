...
export function PublicPortalPage() {
  ...
  return <div className="publicPortal" style={brandStyle(brand)}>
    ...
    {!loading && !error && view === 'home' && <main>
      <section className="publicHero"><div><small>Educação conectada ao agro real</small><h1>Conhecimento para transformar decisões no campo.</h1><p>Cursos, treinamentos corporativos, experiências práticas e tecnologia integrados ao ecossistema iFarm.</p><div className="publicHeroActions"><button className="primary" onClick={() => go('/courses')}>Explorar cursos</button><button onClick={() => go('/events')}>Ver eventos</button></div></div><div className="publicHeroPanel"><span>Smart Farm Experience</span><strong>Aprender. Praticar. Certificar.</strong><p>Conteúdo digital conectado a dias de campo, demonstrações e capacitação prática.</p></div></section>
      ...
      <section className="publicValue"><article><strong>Aprendizado aplicado</strong><p>Conteúdo estruturado para a realidade de produtores, equipes, empresas e parceiros do agro.</p></article><article><strong>Certificados verificáveis</strong><p>Código público, QR Code e metadados preservados na emissão.</p></article><article><strong>Experiência de campo</strong><p>Workshops, dias de campo e atividades práticas conectadas à Smart Farm.</p></article></section>
    ...
    <footer className="publicPortalFooter"><div><strong>{brand.academyName}</strong><span>Educação conectada ao ecossistema iFarm.</span></div><div><button onClick={() => go('/courses')}>Cursos</button><button onClick={() => go('/events')}>Eventos</button><button onClick={() => go('/certificates/validate')}>Certificados</button></div></footer>
  </div>
}
