...
function planAction(plan: PublicPlan) {
  if (plan.commercialMode === 'contact_sales') return { label: 'Entrar e falar com a iFarm', href: '/app' }
  if (plan.commercialMode === 'free') return { label: 'Entrar para começar', href: '/app' }
  if (plan.checkoutReady) return { label: 'Entrar para assinar', href: '/app?view=subscription' }
  return { label: 'Entrar para acompanhar', href: '/app?view=subscription' }
}
...
export function PublicPlansPage() {
  ...
  const action = detail ? planAction(detail) : null
